      ...
      <div className="mx-auto max-w-2xl">
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {fetchedForecast && (
              <div className="flex rounded border border-stone-200 dark:border-stone-600">
                <button
                  type="button"
                  onClick={() => setTempUnit("C")}
                  className={`rounded-l px-2 py-0.5 text-xs font-medium ${tempUnit === "C" ? "bg-stone-200 text-stone-900 dark:bg-stone-600 dark:text-stone-50" : "bg-transparent text-stone-500 dark:text-stone-400"}`}
                >
                  °C
                </button>
                <button
                  ...
                  className={`rounded-r px-2 py-0.5 text-xs font-medium ${tempUnit === "F" ? "bg-stone-200 text-stone-900 dark:bg-stone-600 dark:text-stone-50" : "bg-transparent text-stone-500 dark:text-stone-400"}`}
                >
                  °F
                </button>
              </div>
          ...
          {suggestedForTrip.length > 0 ? (
            <ul className="space-y-2">
              {suggestedForTrip.map((r) => (
                <li
                  key={r.item.id}
                  className="flex items-start justify-between gap-3 rounded-xl border border-stone-200 bg-stone-50 p-3 dark:border-stone-700 dark:bg-stone-800/50"
                >
                  <div>
                    <span className="font-medium text-stone-900 dark:text-stone-50">
                      {r.item.name}
                    </span>
                    <span className="ml-2 text-stone-500">×{r.suggestedQuantity}</span>
            ...
            <p className="rounded-xl border border-stone-200 bg-stone-50 py-3 px-4 text-sm text-stone-500 dark:border-stone-600 dark:bg-stone-800/50 dark:text-stone-400">
        ...
        <section className="mb-8">
          ...
          <ul className="space-y-1.5">
            {essentialsAlways.map((r) => (
              <li
                key={r.item.id}
                className="flex items-center justify-between rounded-lg bg-stone-50 py-2 px-3 dark:bg-stone-800/50"
              >
                <span className="text-stone-800 dark:text-stone-200">
                  {r.item.name}
                </span>
                <span className="text-sm text-stone-500">×{r.suggestedQuantity}</span>
              </li>
            ))}
          </ul>
        </section>
    ...
    <div className="mx-auto max-w-xl">
      <div className="mb-8">
        ...
        <div className="h-1.5 w-full overflow-hidden rounded-full bg-stone-200 dark:bg-stone-700">
          <div
            className="h-full rounded-full bg-stone-700 dark:bg-stone-500 transition-all duration-300"
          ...
          <div className="space-y-5">
            ...
            <div ref={locationWrapperRef} className="relative">
              <input
                ...
                autoComplete="off"
                className="w-full rounded-lg border border-stone-300 bg-white px-4 py-3 text-stone-900 placeholder-stone-400 focus:border-stone-500 focus:outline-none focus:ring-2 focus:ring-stone-400/30 dark:border-stone-600 dark:bg-stone-800 dark:text-stone-100 dark:placeholder-stone-500"
              />
              {locationLoading && (
              ...
              {showLocationDropdown && locationSuggestions.length > 0 && (
                <ul
                  className="absolute z-10 mt-1 max-h-56 w-full overflow-auto rounded-lg border border-stone-200 bg-white py-1 shadow-lg dark:border-stone-600 dark:bg-stone-800"
                  role="listbox"
                >
                  {locationSuggestions.map((loc) => (
                    <li
                      ...
                      className="cursor-pointer px-4 py-2.5 text-sm text-stone-800 hover:bg-stone-100 dark:text-stone-200 dark:hover:bg-stone-700"
                    >
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-stone-300 bg-white px-3 py-2 text-stone-900 focus:border-stone-500 focus:outline-none focus:ring-2 focus:ring-stone-400/30 dark:border-stone-600 dark:bg-stone-800 dark:text-stone-100"
                />
              </div>
              <div>
                <label className="mb-1 block text-xs font-medium text-stone-500 dark:text-stone-400">
                ...
                <button
                  ...
                  className={`rounded-lg border px-4 py-3 text-left text-sm font-medium transition-colors ${
                    form.activities.includes(a.id)
                      ...
                      : "border-stone-200 bg-white text-stone-700 hover:border-stone-300 dark:border-stone-600 dark:bg-stone-800 dark:text-stone-300 dark:hover:border-stone-500"
                  }`}
                >
                  {a.label}
                </button>
                ...
                <button
                  ...
                  className={`rounded-xl border p-4 text-left transition-colors ${
                    form.luggageType === opt.id
                      ? "border-stone-500 bg-stone-100 dark:border-stone-400 dark:bg-stone-700/50"
          ...
          <div className="space-y-6">
            <div>
              <div className="mb-2 flex items-center justify-between gap-2">
                ...
                <div className="flex rounded-lg border border-stone-200 dark:border-stone-600">
                  <button
                    type="button"
                    onClick={() => setTempUnit("C")}
              ...
              {form.latitude != null && form.longitude != null && form.startDate && form.endDate ? (
                <>
                  ...
                  {fetchedForecast && !forecastLoading && (
                    <div className="mb-4 rounded-xl border border-stone-200 bg-stone-50 p-4 dark:border-stone-600 dark:bg-stone-800/50">
